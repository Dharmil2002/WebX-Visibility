  ...
  async addBillDetails(data, bill) {
    ...
    bill.forEach((element) => {
      element?.extraData.filter(item => item.isSelected == true).forEach(nested => {
        ...
        let jsonBilling = {
          ...
          gSTRT: data?.gstExempted == false ? data.gstRate || 0 : 0.00,
          ...
          sGSTRT: gstType.includes(data?.gstType) ? parseFloat(data.gstRate || 0) / 2 : 0,
          cGST: gstType.includes(data?.gstType) ? parseFloat(element.gstCharged) / 2 : 0,
          cGSTRT: gstType.includes(data?.gstType) ? parseFloat(data.gstRate || 0) / 2 : 0,
          uTGST: gstType.includes(data?.gstType) ? parseFloat(element.gstCharged) : 0,
          uTGSTRT: gstType.includes(data?.gstType) ? parseFloat(data.gstRate || 0) : 0,
          iGST: data?.gstType == "IGST" ? element.gstCharged : 0,
          iGSTRT: data?.gstType == "IGST" ? data.gstRate || 0 : 0,
          eNTDT: new Date(),
    ...
    const billData = {
      ...
      "cUST": {
        ...
        "cT": custList[0]?.city || "",
        "sT": custList[0]?.state || data?.cState,
        "gSTIN": data?.cGstin || "",
