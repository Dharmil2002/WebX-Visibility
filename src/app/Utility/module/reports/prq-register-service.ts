...
export class prqreportService {
    ...
    async getprqReportDetail(start, end, billparty, prqArray) {
        ...
        res.data.forEach(item => {
            item.prqDt = item.prqDt ? moment(item.prqDt).format("DD MMM YY") : "";
            item.PickDtTime = item.PickDtTime ? moment(item.PickDtTime).format("DD MMM YY HH:mm") : "";
            item.ConsigNoteDt = item.ConsigNoteDt ? moment(item.ConsigNoteDt).format("DD MMM YY") : "";
            item.THCDt = item.THCDt ? moment(item.THCDt).format("DD MMM YY") : "";
        });
        return res.data;
    }
}
