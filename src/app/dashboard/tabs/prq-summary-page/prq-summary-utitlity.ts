...
export async function getPrqDetailFromApi(masterServices) {
    ...
    prqData.map((element, index) => {
        let pqrData = {
            ...
            "status": element?.status === "0" ? "Awaiting Confirmation" : element.status === "1" ? "Awaiting Assign Vehicle" :element.status=="2"?"Awaiting For Docket":"Ready For THC",
            "actions": element?.status === "0" ? ["Confirm", "Reject", "Modify"] : element.status === "1" ? ["Assign Vehicle"] :element.status=="2"?["Add Docket"]:["Add Docket","Create THC"],
